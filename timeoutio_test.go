...
func Test_ReadTimeout(t *testing.T) {
	...
	defer c.Close()
	reader := newTimeoutReader(c, 16*1024, -1*time.Second)
...
func Test_WriteTimeout(t *testing.T) {
	...
	defer c.Close()
	writer := newTimeoutWriter(c, 16*1024, -1*time.Second)
...
func Test_ReadByte(t *testing.T) {
	...
	reader := newTimeoutReader(c, 16*1024, 2*time.Second)
...
func Test_WriteByte(t *testing.T) {
	c := new(testConn)
	writer := newTimeoutWriter(c, 16*1024, 2*time.Second)

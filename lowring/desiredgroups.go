...
type desiredGroups struct {
	byDesire []int
	toDesire []int32
	RandIntn func(int) int
}

func newDesiredGroups(cap int, randIntn func(int) int) *desiredGroups {
	return &desiredGroups{
		byDesire: make([]int, 0, cap),
		toDesire: make([]int32, 0, cap),
		RandIntn: randIntn,
	}
}

func (x *desiredGroups) Add(key int, value int32) {
	ln := len(x.toDesire)
	if int(key) < ln {
		x.Move(key, value)
		return
...
func (x *desiredGroups) Move(key int, value int32) {
	var oldPosition int
...
func (x *desiredGroups) String() string {
	s := "["

...
func Example_tiers() {
	...
	//     Replica 1 assigned to firstDisk of serverOne
	partition = 1
	for replica := 0; replica < builder.ReplicaCount(); replica++ {
		fmt.Printf("    Replica %d assigned to %s of %s\n", replica,
			nodeNames[builder.Ring[replica][partition]],
	...
	fmt.Println("If the node name (disk name) happens to be the same it doesn't matter since they are on different servers.")
